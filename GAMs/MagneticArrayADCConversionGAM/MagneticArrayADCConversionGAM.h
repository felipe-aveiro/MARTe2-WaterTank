/**
 * @file MagneticArrayADCConversionGAM.h
 * @brief Header file for class MagneticArrayADCConversionGAM
 * @date 19/07/2025
 ...
 * @details This header file contains the declaration of the class MagneticArrayADCConversionGAM
...
#ifndef MAGNETICARRAYADCCONVERSIONGAM_H_
#define MAGNETICARRAYADCCONVERSIONGAM_H_
 ...
 *
 * ### Key Features:
 * - Real-time conversion of a single ADC integrated input signal vector of 16 float32 elements (only first 12 used) to magnetic field values
 * - Performs real-time processing for plasma control applications
 * - Outputs 12 signals (one for each Mirnov coil)
...
* ```
 * +MagneticArrayADCConversionGAM = {
 *   Class = MagneticArrayADCConversionGAM
...
class MagneticArrayADCConversionGAM : public MARTe::GAM {
public:
     ...
     */
    virtual ~MagneticArrayADCConversionGAM();
     ...
     * @details Ensures that the number of input signals equals 1u and output signals equals NUM_SIGNALS = 12u
     * and validates samples, elements, and dimensions (must be 1, 1, 0, respectively, for output signals, and 1, 16, 1 respectively, for the input signal).
     * @return true if all conditions are satisfied.
     */
    virtual bool Setup();

    /**
     * @brief Converts ADC integrated signals from the ISTTOK ADCIntegF input signal vector of 16 float32 elements to magnetic fields.
...
private:
    ...
    /**
     * Pointer to input vector (16 elements)
     */
    MARTe::float32 *inputSignal;
    /**
     * Output signals pointers (12 elements)
     */
...
#endif /* MAGNETICARRAYADCCONVERSIONGAM_H_ */
