...
histogram_files = {
    "Shot 48555": "/home/felipe/git-repos/MARTe2-WaterTank/DataVisualization/Outputs/IsttokOutput_DataFusion_48555_histogram.csv",
    "Shot 53071": "/home/felipe/git-repos/MARTe2-WaterTank/DataVisualization/Outputs/IsttokOutput_DataFusion_53071_histogram.csv",
}

export_path = "/home/felipe/git-repos/MARTe2-WaterTank/DataVisualization/Outputs/EstimationHistogramPlots/Estimation_Fusion_CycleTime_Histograms.png"

# HistogramGAM configuration
min_lim = 80.0      # microseconds
max_lim = 120.0     # microseconds
...
for x in tick_edges:
    ...
    if label in ["85.0", "90.0", "95.0", "100.0", "105.0", "110.0", "115.0"]:
...
for idx, (shot_name, shot_percentages) in enumerate(shot_items):
    ...
    for i, (x, y, count) in enumerate(zip(x_values, shot_percentages, shot_counts)):
        ...
        if i == len(x_values) - 1:
            text = pg.TextItem(f"{y:.2f}%", anchor=(0.5, 0.0))
            text.setFont(value_font)
            text.setPos(x, max(y - 4.0, 2.0))
            plot.addItem(text)
